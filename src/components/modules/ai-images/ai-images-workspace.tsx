...
import { generateImage, editImage, saveGeneratedImage } from "@/lib/api/actions/ai-images";
...
import { AiImageGallery } from "@/components/modules/ai-images/ai-image-gallery";
...
export function AiImagesWorkspace() {
  ...
  }, [mode, prompt, model, aspectRatio, imageSize, editFile, refreshGallery]);
  ...
  const handleLoadSettingsFromGallery = useCallback((img: SavedGeneratedImage) => {
    ...
    toast.success("설정을 불러왔습니다. 생성 탭으로 이동합니다.");
  }, []);

  return (
    <>
      <div className="mt-4 flex gap-1 rounded-lg border border-[#2a2a2a] bg-[#141414] p-1">
        <button
          type="button"
          onClick={() => setActiveTab("generate")}
          className={`flex flex-1 items-center justify-center gap-2 rounded-md py-2.5 text-sm font-medium transition-colors ${
            activeTab === "generate" ? "bg-[#e5e5e5] text-[#0a0a0a]" : "text-[#9a9a9a] hover:text-[#d4d4d4]"
          }`}
        >
          <Sparkles className="h-4 w-4" /> 이미지 생성
        </button>
        <button
          type="button"
          onClick={() => setActiveTab("gallery")}
          className={`flex flex-1 items-center justify-center gap-2 rounded-md py-2.5 text-sm font-medium transition-colors ${
            activeTab === "gallery" ? "bg-[#e5e5e5] text-[#0a0a0a]" : "text-[#9a9a9a] hover:text-[#d4d4d4]"
          }`}
        >
      ...
      {activeTab === "generate" ? (
          ...
          <div className="space-y-4">
            ...
            {mode === "edit" && (
              <Card className="space-y-3 border-[#2a2a2a] bg-[#141414] p-4">
                ...
                {editPreview && (
                  <Button variant="ghost" size="sm" onClick={() => setEditFile(null)} className="w-full">
                    <X className="mr-1 h-3 w-3" /> 이미지 제거
                  </Button>
                )}
              </Card>
            )}
            ...
            <div className="space-y-1.5 rounded-md border border-[#2a2a2a] bg-[#141414] px-4 py-3">
              ...
              <ul className="space-y-1 text-[11px] text-[#9a9a9a]">
                <li>&bull; 장면을 서술적으로 묘사하세요</li>
                <li>&bull; &quot;85mm 렌즈&quot;, &quot;골든 아워&quot; 같은 사진 용어 활용</li>
                <li>&bull; 텍스트가 필요하면 정확한 문구와 폰트 스타일 명시</li>
                <li>&bull; &quot;~하지 말고&quot;보다 &quot;~하게&quot;로 표현하세요</li>
                <li>&bull; Flash는 빠르고, Pro는 더 정교합니다</li>
              </ul>
            </div>
          </div>
              ...
              {generating ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setActiveTab("gallery")}>
                      <ImageIcon className="mr-1 h-3.5 w-3.5" /> 갤러리 보기
                    </Button>
                  </div>
                </div>
              ) : (
        ...
        <AiImageGallery onLoadSettings={handleLoadSettingsFromGallery} />
      )}
