...
@pytest.mark.asyncio
async def test_on_proposal_created_sends_discord(notifier):
...
@pytest.mark.asyncio
async def test_on_discord_reaction_approve(notifier):
    executor = MagicMock()
...
@pytest.mark.asyncio
async def test_on_discord_reaction_reject(notifier):
    workflow = ApprovalWorkflow(notifier=notifier)
...
@pytest.mark.asyncio
async def test_on_discord_reaction_modify(notifier):
    workflow = ApprovalWorkflow(notifier=notifier)
...
@pytest.mark.asyncio
async def test_on_dashboard_action_approve_notifies_discord(notifier):
    executor = MagicMock()
    executor.execute = AsyncMock(return_value=SimpleNamespace(success=True))
    workflow = ApprovalWorkflow(notifier=notifier, executor=executor)
